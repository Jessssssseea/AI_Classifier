...
def send_toast(subject, file_path):
    ...
    # 创建 XML 模板
    toast_xml = notifications.ToastNotificationManager.get_template_content(notifications.ToastTemplateType.TOAST_TEXT02)
...
class FileHandler(FileSystemEventHandler):
    ...
    def handle_user_feedback(self, subject, file_path):
        ...
        try:
            ...
            user_feedback = "yes"  # 如果文件不存在，默认认为用户没有点击通知
...
def move_back(file_path):
    subject = classify_file(file_path)
    filename = os.path.basename(file_path)
    original_path = os.path.join(config["WATCH_FOLDER"], filename)
    new_path = os.path.join(config["OUTPUT_BASE_FOLDER"]+'/'+subject, filename)
    # print(new_path)
    # print(original_path)
    # print(filename)
    # print(file_path)
